...
function Randomat:CanEventRun(event, ignore_history)
    ...
    if type(can_run) == "boolean" then return can_run, reason or "Blocked by 'TTTRandomatCanEventRun' hook" end
...
function Randomat:IsEventCategoryActive(category)
    ...
    for _, e in pairs(Randomat.ActiveEvents) do
        if type(e.Categories) == "table" and table.HasValue(e.Categories, lower_cat) then return true end
    end
...
function Randomat:SetRole(ply, role, set_max_hp, scale_hp)
    ...
    if old_role ~= role and isfunction(CRVersion) and not CRVersion("2.5.4") then
